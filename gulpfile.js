...
gulp.task('es5-build', () => {
    ...
    gulp.src(src).pipe(concat('selftimer.js')).pipe(gulp.dest(source + 'build/es5/'));
});
...
gulp.task('es5-minify', () => {
    ...
    gulp.src('src/build/es5/intermediate/selftimer.js').pipe(minify({
        ext: {
            src: '.js',
            min: '.min.js'
        },
...
gulp.task('promise-build', () => {
    ...
    let files = function(path) {
        return [
            'header', 'utils/helpers', 'utils/messages', 'utils/formats', 'methods/' + path + '/on',
            'methods/' + path + '/at',
            'methods/' + path + '/in',
            'methods/' + path + '/is'
        ];
    ...
    gulp.src(src_promise).pipe(concat('selftimer.js')).pipe(gulp.dest(source + 'build/promise/'));
...
gulp.task('promise-polyfill-minify', () => {
   ...
   * minify
   */
    gulp.src('src/build/promise/intermediate2/selftimer.js').pipe(minify({
        ext: {
...
gulp.task('promise-minify', () => {
   ...
   * minify
   */
    gulp.src('src/build/promise/intermediate/selftimer.js').pipe(minify({
        ext: {
            src: '-promise.js',
            min: '-promise.min.js'
        },
